...
test.describe('Inventory Page Test', () => {
  ...
  test('verify all product images load correctly', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      const isLoaded = await inventory.imageLoaded(i);
      if (!isLoaded) {
        console.log(`Image at index ${i} failed to load.`);
      }
      expect.soft(isLoaded, `Image at index ${i} should be loaded`).toBeTruthy();
    }
